...
func main() {
	bPathArr := strings.Split(bPath, ",")
	aPathArr := strings.Split(aPath, ",")
	//intersections := []string{}
	aVisited := unique(getVisitedPoints(aPathArr))
	bVisited := unique(getVisitedPoints(bPathArr))
	intersection := HashIntersection(aVisited, bVisited)
	var intersectionPoints []Point
	for _, val := range intersection{
		valArr := strings.Split(val, ",")
		xVal, _ := strconv.Atoi(valArr[0])
		yVal, _ := strconv.Atoi(valArr[1])
	...
	for _, point := range intersectionPoints {
		...
		distCheck := (point.x * point.x) + (point.y * point.y)
...
func getVisitedPoints(pathArr []string) []string {
	...
	for _, val := range pathArr {
		...
		for x := 0; x < distance; x++ {
			lastPoint := visitedPoints[len(visitedPoints)-1]
			visitedPoints = append(visitedPoints, getPointToAdd(dir, lastPoint))
		}
	}
	return visitedPoints
}

func changeX(current string, amount int) string{
	xAndY := strings.Split(current,",")
...
func changeY(current string, amount int) string{
	xAndY := strings.Split(current,",")
	x,_ := strconv.Atoi(xAndY[0])
	y,_ := strconv.Atoi(xAndY[1])
	y+=amount
...
func getPointToAdd(dir string, lastPoint string) string {
	switch dir {
	case "L":
		return changeX(lastPoint, -1)
	case "R":
		return changeX(lastPoint, 1)
	case "U":
		return changeY(lastPoint, 1)
	default:
		return changeY(lastPoint, -1)
	}
}
